#ifndef INFO_ORGANIZATION_H_INCLUDED
#define INFO_ORGANIZATION_H_INCLUDED
...
//Return Type:  node
TreeNode* create_tree_node(int date, int tickID, int platform_num, char train_name[30], int persons, float price, char station[30], char seat[5], char train_class[20], char from[20], char to[20])
{
...
//Return Type:  root
TreeNode* insert(TreeNode* root,int date, int tickID, int platform_num, char train_name[30], int persons, float price, char station[30], char seat[5], char train_class[20], char from[20], char to[20])
{
    if (root == NULL)
    {
        return create_tree_node(date, tickID, platform_num, train_name, persons, price, station, seat, train_class, from, to);
    }

    if (tickID < root->tickID)
    {
        root->left = insert(root->left, date, tickID, platform_num, train_name, persons, price, station, seat, train_class, from, to);
    }
    else if (tickID > root->tickID)
    {
        root->right = insert(root->right, date, tickID, platform_num, train_name, persons, price, station, seat, train_class, from, to);
    }

    return root;
}

//Function:     search_tickid
//Description:  shows information necessary to passenger with tick-id(101,102,etc ->prototype )
//Input param:  root pointer
//Return Type:  NULL
void search_tickid(TreeNode* root,int tick)
{
    ...
    if (root != NULL)
    {
        ...
        search_tickid(root->left,tick);
        search_tickid(root->right,tick);
    }
}
...
TreeNode* inputs(TreeNode* root)
{
    ...
    for (i = 0; i < in; i++)
    {
        ...
        float price;
        char train_name[30], station[30], seat[5], train_class[20], from[20], to[20];
        ...
        root = insert(root, date, tickID, platform_num, train_name, persons, price, station, seat, train_class, from, to);
    }
...
// Return Type: root
TreeNode* delete_train(TreeNode* root, int tick)
{
    ...
    if (tick < root->tickID)
    {
        root->left = delete_train(root->left, tick);
    }
    else if (tick > root->tickID)
    {
        root->right = delete_train(root->right, tick);
    }
    else
    {
        if (root->left == NULL)
        ...
        strcpy(root->train_class, temp->train_class);
        strcpy(root->from, temp->from);
        strcpy(root->to, temp->to);

        root->right = delete_train(root->right, temp->tickID);
    }
    ...
    do
    {
        ...
        case '2':
            ...
            if (opt == 'y')
            {
                ...
                scanf("%d", &tick_del);
                root = delete_train(root, tick_del);
...
#endif // INFO_ORGANIZATION_H_INCLUDED
